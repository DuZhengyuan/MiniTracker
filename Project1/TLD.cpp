...
void TLD::generatePositiveData(const Mat& frame, int num_warps){
  ...
  for (int i=0;i<num_warps;i++){
     if (i>0)
       ...
       for (int b=0;b < (int)good_boxes.size();b++){
         idx=good_boxes[b];
		 patch = img(grid[idx]);
 ...
 */
  ...
  Mat patch;
  for (int j=0;j < (int)bad_boxes.size();j++){
      idx = bad_boxes[j];
...
void TLD::detect(const cv::Mat& frame){
  ...
  for (int i=0;i < (int)grid.size();i++){//FIXME: BottleNeck
...
//  imshow("detections",img);
  ...
  if (dbb.size()>0){
      ...
      if(DXY_PRINTF) printf("No NN matches found.\n");
      detected=false;
  }
}
...
/// Data generation
  ...
  int idx;
  for (int i=0;i < (int)bad_boxes.size();i++){
      idx=bad_boxes[i];
      if (tmp.conf[idx]>=1){
  ...
  for (int i=0;i < (int)dt.bb.size();i++){
      idx = dt.bb[i];
